...
def test_math_residue_is_classified_not_counted_as_missing() -> None:
    """一个 'X' 查不到，不等于内容丢了——那是数学字体的残渣。

    把它算进"必需缺失"，一篇干净的论文会永远停在"交给人"，
    而人打开一看只是个孤立的字符。如实分类：不计入必需完整性，
    证据里写明原内容已随所在区域整块保留，供人复查。
    """
    ...
    residue = [
        item
        for item in mapping.locations
        if item.method == METHOD_NO_EVIDENCE and "残渣" in item.evidence
    ]
    assert residue
    for item in residue:
        assert item.required is False
        assert not item.located
