...
def test_the_real_unlocatable_units_are_handed_to_a_human() -> None:
    mapping, audit = _real_inputs()
    plan = plan_repair(mapping, audit)
    signals = {item.signal for item in plan.manual}
    assert "required-without-evidence" in signals
    handled = {item.element_id for item in plan.actions}
    for item in plan.manual:
        assert item.element_id not in handled
